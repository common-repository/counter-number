<?php
function con_6310_link_css_js($hook)
{
   if(
      $hook != 'toplevel_page_con-6310-counter-number' && 
      $hook != 'counter-number_page_con-6310-template-01-10' && 
      $hook != 'counter-number_page_con-6310-template-11-20' && 
      $hook != 'counter-number_page_con-6310-counter-number-manage-items' && 
      $hook != 'counter-number_page_con-6310-counter-number-import-export' && 
      $hook != 'counter-number_page_con-6310-counter-number-license' && 
      $hook != 'counter-number_page_con-6310-counter-number-use' && 
      $hook != 'counter-number_page_con-6310-wpmart-plugins' 
   ) {
   
   return;
}
   wp_enqueue_style('con-google-font', 'https://fonts.googleapis.com/css?family=Amaranth');
   wp_enqueue_style('con-6310-style', plugins_url('assets/css/style.css', __FILE__));
   wp_enqueue_style('con-6310-color-style', plugins_url('assets/css/jquery.minicolors.css', __FILE__));
   wp_enqueue_style('con-6310-font-select-style', plugins_url('assets/css/fontselect.css', __FILE__));
   wp_enqueue_style('con-6310-font-awesome-5-0-13', 'https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.9.0/css/all.min.css');
   wp_enqueue_style('con-6310-codemirror-style', 'https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.48.0/codemirror.min.css');
   wp_enqueue_style('con-6310-color-style', 'https://cdnjs.cloudflare.com/ajax/libs/jquery-minicolors/2.3.4/jquery.minicolors.min.css');
   wp_enqueue_style('con-6310-jquery-ui-css', "https://cdnjs.cloudflare.com/ajax/libs/jqueryui/1.12.1/jquery-ui.min.css");
   wp_enqueue_style('con-6310-style-01-10', plugins_url('assets/css/style-01-10.css', __FILE__));
   wp_enqueue_style('con-6310-style-11-20', plugins_url('assets/css/style-11-20.css', __FILE__));   
   wp_enqueue_style('con-6310-style-contact-description', plugins_url('assets/css/contact-description.css', __FILE__));
   
   wp_enqueue_script('con-6310-counterup', plugins_url('assets/js/jquery.counterup.js', __FILE__), array('jquery'), con_6310_PLUGIN_CURRENT_VERSION, true);
   wp_enqueue_script('con-6310-font-select-js', plugins_url('assets/js/fontselect.js', __FILE__), array('jquery'));
   wp_enqueue_script('con-6310-jquery-ui-js', 'https://cdnjs.cloudflare.com/ajax/libs/jqueryui/1.12.1/jquery-ui.min.js', array('jquery'));
   wp_enqueue_script('con-6310-color-js', 'https://cdnjs.cloudflare.com/ajax/libs/jquery-minicolors/2.3.4/jquery.minicolors.min.js', array('jquery'));
   wp_enqueue_script('con-6310-codemirror-js', 'https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.48.0/codemirror.min.js', array('jquery'));
   wp_enqueue_script('con-6310-font-select-js', plugins_url('assets/js/fontselect.js', __FILE__), array('jquery'));
   wp_enqueue_script('con-6310-admin-js', plugins_url('assets/js/con-6310-admin-script.js', __FILE__), array('jquery'));
   wp_enqueue_script('con-6310-modal-js', plugins_url('assets/js/con-6310-admin-modal.js', __FILE__), array('jquery'));
}

function con_6310_replace($data) {
	if($data == '') return ''; 
...
function con_6310_get_user_roles(){
...
function con_6310_extract_data($data)
{
   $array_key = "";
   $array_value = "";
   $i = 0;
...
function con_6310_counter_number_install()
   {
      global $wpdb;
      global $con_6310_counter_number_version;
      ...
      $sql2 = "CREATE TABLE IF NOT EXISTS $item_table (
        ...
        endnumbertype text DEFAULT NULL,
        selectoption varchar(100) DEFAULT -1,
        commentpost text DEFAULT NULL,
        customquery text DEFAULT NULL,
        leadingnumber text DEFAULT 0,
        sideimage text DEFAULT NULL, 
      ...
      if (!($itemData)) {
         $url = con_6310_plugin_dir_url;
         $details = "Lorem Ipsum is simply dummy text of the printing and typesetting industry.";

         $wpdb->query("INSERT INTO {$item_table} 
         (title, description, numbers_start, numbers, icontype, icons, hovericons, image, hoverimage, targertype, detailstext, detailsurl, commons, endnumbertype, selectoption, commentpost, customquery, leadingnumber, sideimage, styleid) 
         VALUES ('University','{$details}','1','350','1','fas fa-school','fas fa-award','','','1','read more','https://www.pluginbd.com','','1','','','','','https://www.pluginsbd.com/wp-content/plugin-images/con/1.jpg',''), 
         ('Message','{$details}','200','789','1','fas fa-envelope','fas fa-fax','','','1','read more','https://www.pluginbd.com','','1','','','','','https://www.pluginsbd.com/wp-content/plugin-images/con/9.jpg',''),
         ('Cell Phone','{$details}','500','358','1','fas fa-phone-square-alt','fas fa-phone-alt','','','1','read more','https://www.pluginbd.com','','1','','','','','https://www.pluginsbd.com/wp-content/plugin-images/con/10.jpg','')   				         
         ");
      }
   }

   function con_6310_get_option($name) {
		global $wpdb;
   ...
   function con_6310_version_status() {
      global $wpdb;
      $db_version = con_6310_get_option('con_6310_version_info');
      if(!$db_version){
         $wpdb->query("DELETE FROM {$wpdb->prefix}options where option_name='con_6310_version_info'");
            $wpdb->query("INSERT INTO {$wpdb->prefix}options(option_name, option_value) VALUES ('con_6310_version_info', '".con_6310_PLUGIN_CURRENT_VERSION."')");
      }
      else{
         $key = con_6310_get_option('con_6310_license_key');
         if($db_version != con_6310_PLUGIN_CURRENT_VERSION && $key){
            con_6310_check_license($key, true);
            $wpdb->query("UPDATE {$wpdb->prefix}options set 
                        option_value='". con_6310_PLUGIN_CURRENT_VERSION ."' 
                        where option_name = 'con_6310_version_info'");
         }	
      }
   } 
   
   function con_6310_check_license($key, $autoUpdate = false)
   {
      global $wpdb;
   
      $db_key = con_6310_get_option('con_6310_license_key');
      if(!$db_key){
         $wpdb->query("DELETE FROM {$wpdb->prefix}options where option_name='con_6310_license_key'");
         $wpdb->query($wpdb->prepare("INSERT INTO {$wpdb->prefix}options SET option_name = %s, option_value = %s", 'con_6310_license_key', "{$key}"));
         if(!$wpdb->insert_id) {
            $wpdb->query($wpdb->prepare("UPDATE {$wpdb->prefix}options SET option_value = %s where option_name = %s", "{$key}", 'con_6310_license_key'));
         }
      }else if($db_key != $key){
         $wpdb->query($wpdb->prepare("UPDATE {$wpdb->prefix}options SET option_value = %s where option_name = %s", "{$key}", 'con_6310_license_key'));
      }
      ...
      if(!class_exists('ZipArchive')){
         $api_params = array(
            ...
            'license' => $key,
            'item_name' => urlencode('Counter Number'),
            'url' => home_url(),
            'type' => 'con'
         );
         ...
         if (is_wp_error($response) || 200 !== wp_remote_retrieve_response_code($response)) {
            ...
            if (false === $license_data->success) {
               switch ($license_data->error) {
                  case 'invalid_key':
                     $message = __('<p class="con-6310-error-message">Your have enter invalid license key.</p>');
                     break;
                  case 'site_inactive':
                     $message = __('<p class="con-6310-error-message">Your license is not active for this URL.</p>');
                     break;
                  default:
                     $message = __('<p class="con-6310-error-message">An error occurred, please try again.</p>');
                     break;
               }
               return;
            }
         ...
         if (is_wp_error($unzipfile)) {
            echo '<p class="con-6310-error-message">There was an error unzipping the file.</p>';
            return;
         } else {
            
            if(!$autoUpdate){
               echo "<p class='con-6310-success-message'>Congratulations! Your license activated successfully.</p>";		
   ...
   function con_6310_fa_icon_list($startTag, $endTag) {
      $iconArray = array (
   ...
   function con_6310_extract_item($items)
   {
      global $wpdb;
      $item_table = $wpdb->prefix . 'con_6310_item';
      $results = array();
...
function con_6310_display_icon($templateId, $type, $icons, $hover_icons, $image, $hover_image) {
   echo "
      <div class='con-6310-template-".esc_attr($templateId)."-icon-wrapper'>
         <div class='con-6310-template-".esc_attr($templateId)."-icon'>";
         if($type == 1) {
            $hover_icons = $hover_icons ? $hover_icons : $icons;
            echo "
                  <i class='".esc_attr($icons)." con-6310-icon'></i>
                  <i class='".esc_attr($hover_icons)." con-6310-hover-icon'></i>
               ";
         } else{
            $hover_image = $hover_image ? $hover_image : $image;
            echo "
                  <img src='".esc_attr($image)."' class='con-6310-image'>
                  <img src='".esc_attr($hover_image)."' class='con-6310-hover-image'>
               ";
         }
   echo  "</div>
      </div>
...
function con_6310_display_icon_free_output($templateId, $type, $icons, $hover_icons, $image, $hover_image) {
   echo "
      <div class='con-6310-template-".esc_attr($templateId)."-icon-wrapper'>
         <div class='con-6310-template-".esc_attr($templateId)."-icon'>";
...
function con_6310_display_read_more($templateId, $target, $text, $url, $output = 1) {
   if(!strlen($text)) return;
   if(!$output) return;
   $target = $target ? "target='_blank'" : '';
   echo "
   <div class='con-6310-template-".esc_attr($templateId)."-read-more'>
      <a href='{$url}' {$target}>{$text}</a>
   </div>
   ";
}

function con_6310_general_form_left($bgType, $cssData) {
   wp_enqueue_media();
   ...
   ?>
      <tr>
         ...
         <td>
         <select name="desktop_item_per_row" class="con-6310-form-input con-6310-no-preview">
            <?php
               for($i = 1; $i <= 6; $i++){
      ...
      <tr>
         <td width="55%">
         <b>Tablet Item Per Row</b>  <span class="con-6310-pro">(Pro) <div class="con-6310-pro-text">This feature is available on the pro version only. You can view changes in the admin panel, not in the output.</div></span>
         <div class="con-6310-no-live-preview">Live Preview Not Available</div>
         </td>
         <td>
         <select name="tablet_item_per_row" class="con-6310-form-input con-6310-no-preview">
            <?php
               for($i = 1; $i <= 3; $i++){
      ...
      <tr>
         <td width="55%">
         <b>Mobile Item Per Row</b>  <span class="con-6310-pro">(Pro) <div class="con-6310-pro-text">This feature is available on the pro version only. You can view changes in the admin panel, not in the output.</div></span>
         <div class="con-6310-no-live-preview">Live Preview Not Available</div>
         </td>
         <td>
         <select name="mobile_item_per_row" class="con-6310-form-input con-6310-no-preview">
            <?php
               for($i = 1; $i <= 3; $i++){
      ...
      <tr height="45">
         <td width="55%">
            <b>Template Background Type</b> <span class="con-6310-pro">(Pro) <div class="con-6310-pro-text">This feature is available on the pro version only. You can view changes in the admin panel, not in the output.</div></span>
            <div class="con-6310-no-live-preview">Live Preview Not Available</div>
         </td>
         <td>
            <select name="background_type" id="background_type" class="con-6310-form-input">
               <?php
      ...
      <tr height="45" class='background-type-2'>
         <td width="55%">
            <b>Template Background Color</b> <span class="con-6310-pro">(Pro) <div class="con-6310-pro-text">This feature is available on the pro version only. You can view changes in the admin panel, not in the output.</div></span>
            <div class="con-6310-no-live-preview">Live Preview Not Available</div>
         </td>
         <td>
            <input type="text" name="template_background_color" class="con-6310-form-input con_6310_color_picker" step="1" data-opacity="0" data-format="rgb" value="<?php echo isset($cssData['template_background_color']) ? esc_attr($cssData['template_background_color']) : 'rgba(255, 255, 255, 0)' ?>">
         </td>
      </tr>
      <tr height="45" class='background-type-3'>
         <td width="55%">
            <b>Template Background Image</b> <span class="con-6310-pro">(Pro) <div class="con-6310-pro-text">This feature is available on the pro version only. You can view changes in the admin panel, not in the output.</div></span>
            <div class="con-6310-no-live-preview">Live Preview Not Available</div>
         </td>
         <td>
         <input type="text" name="box_background_image" id="con_6310_box_background_image" class="con-6310-form-input" data-opacity=".8" data-format="rgb" value="<?php echo isset($cssData['box_background_image']) ? esc_attr($cssData['box_background_image']) : '' ?>">
         <input type="button" id="con_6310_box_background_image_button" value="Upload Image" class="con-6310-btn-default" style='margin-top: 5px;'>
         </td>
      </tr>
      <tr height="45" class='background-type-3'>
         <td width="55%">
            <b>Image Opacity Color</b> <span class="con-6310-pro">(Pro) <div class="con-6310-pro-text">This feature is available on the pro version only. You can view changes in the admin panel, not in the output.</div></span>
            <div class="con-6310-no-live-preview">Live Preview Not Available</div>
         </td>
         <td>
            <input type="text" name="image_opacity" class="con-6310-form-input con_6310_color_picker" step="1" data-opacity="0" data-format="rgb" value="<?php echo isset($cssData['image_opacity']) ? esc_attr($cssData['image_opacity']) : 'rgba(255, 255, 255, 0)' ?>">
         </td>
      </tr>
      <tr height="45" class='background-type-4'>
         <td width="55%">
            <b>Youtube Video URL</b> <span class="con-6310-pro">(Pro) <div class="con-6310-pro-text">This feature is available on the pro version only. You can view changes in the admin panel, not in the output.</div></span>
            <div class="con-6310-no-live-preview">Live Preview Not Available</div>
         </td>
         <td>
         <input type="text" name="youtube_video_url" class="con-6310-form-input" value="<?php echo isset( $cssData['youtube_video_url']) ? esc_attr($cssData['youtube_video_url']) : '' ?>">
         </td>
      </tr>
      <tr height="45" class='background-type-4'>
         <td width="55%">
            <b>Video Opacity</b> <span class="con-6310-pro">(Pro) <div class="con-6310-pro-text">This feature is available on the pro version only. You can view changes in the admin panel, not in the output.</div></span>
            <div class="con-6310-no-live-preview">Live Preview Not Available</div>
         </td>
         <td>
         <input type="number" name="video_opacity" class="con-6310-form-input" value="<?php echo isset($cssData['video_opacity']) ? esc_attr($cssData['video_opacity']) : '.7' ?>" step='.01' min='0' max='1'>
         </td>
      </tr>
      <tr height="45" class='background-type-4'>
         <td width="55%">
            <b>Video Opacity Color</b> <span class="con-6310-pro">(Pro) <div class="con-6310-pro-text">This feature is available on the pro version only. You can view changes in the admin panel, not in the output.</div></span>
            <div class="con-6310-no-live-preview">Live Preview Not Available</div>
         </td>
         <td>
            <input type="text" name="video_opacity_color" class="con-6310-form-input con_6310_color_picker" step="1" data-opacity="0" data-format="rgb" value="<?php echo isset($cssData['video_opacity_color']) ? esc_attr($cssData['video_opacity_color']) : 'rgba(255, 255, 255, 0)' ?>">
         </td>
      </tr>
      <tr height="45">
         <td>
            <b>Animation Duration</b>  <span class="con-6310-pro">(Pro) <div class="con-6310-pro-text">This feature is available on the pro version only. You can view changes in the admin panel, not in the output.</div></span>
            <div class="con-6310-no-live-preview">Live Preview Not Available</div>
         </td>
         <td>
         <?php
         ...
         ano_6310_create_dropdown($list, 'animation_duration', $cssData['animation_duration']);
         ?>
      </td>
      </tr>
      <tr height="45">
         <td>
            <b>Thousands separator</b>  <span class="con-6310-pro">(Pro) <div class="con-6310-pro-text">This feature is available on the pro version only. You can view changes in the admin panel, not in the output.</div></span>
            <div class="con-6310-no-live-preview">Live Preview Not Available</div>
            </td>
            <td>
            <input type="text" name="thousands_separator" value="<?php echo con_6310_special_character_remove(isset($cssData['thousands_separator']) ? $cssData['thousands_separator'] : '') ?>" class="con-6310-form-input">
         </td>
      </tr>
   <?php
}

function con_6310_add_new_media($id, $items = NULL)
   {
      global $wpdb;
      wp_enqueue_media();
      $style_table = $wpdb->prefix . 'con_6310_style';
      $item_table = $wpdb->prefix . 'con_6310_item';
?>
      <div class="con_6310_add_media">
         <h6>Shortcode</h6>
         <div class="con_6310_add_media_body_shortcode">
            <input type="text" onclick="this.setSelectionRange(0, this.value.length)" value='[con_6310_counter_number id="<?php echo esc_attr($id); ?>"]' />
         </div>
      </div>
      <br />
      <div class="con_6310_add_media">
         <h6>Customize Items</h6>
         <div class="con_6310_add_media_body" id="con-6310-add-or-edit-items">
            <i class="fas fa-plus-circle con_6310_add_media_add_new_icon"></i>
            Add/Edit Items
         </div>
      </div>
      <br />
      <div class="con_6310_add_media">
         ...
         <div class="con_6310_add_media_body" id="con-6310-rearrange-items">
            <i class="fas fa-user-cog con_6310_add_media_add_new_icon"></i>
         </div>
      </div>
      <br />
      <div class="con_6310_add_media">
         <h6>How to Use</h6>
         <div class="con_6310_add_media_body">
      ...
      <div id="con-6310-add-or-edit-items-modal" class="con-6310-modal con-6310-display-none">
         <div class="con-6310-modal-content con-6310-modal-md">
            <form action="" method="post">
               ...
               <div class="con-6310-modal-body-form">
                  ...
                  <?php wp_nonce_field("con-6310-nonce-add-edit-item") ?>
                  <table border="0" cellpadding="0" cellspacing="0" class="con-6310-item-table">
                     ...
                     if (!$itemList) {
                        echo "<tr height='60'>
                           <td colspan='4'>No team items found. For add Counter Number, <a href='" . site_url() . "/wp-admin/admin.php?page=con-6310-counter-number-manage-items' class='con-6310-btn-success'>Click Here</a></td>
                     </tr>";
                     } else {
                     ...
                     ?>
                           <tr class="con-6310-row-select" id="<?php echo esc_attr($allItber['id']); ?>">
                              <td width="20"><input type="checkbox" name="item_id[]" value="<?php echo esc_attr($allItber['id']) ?>" <?php echo esc_attr($checked); ?> id="chk-box-<?php echo esc_attr($allItber['id']); ?>" class="con-6310-row-select-checkbox" /></td>
                              <td><?php echo con_6310_replace(esc_attr($allItber['title'])) ?></td>
                              <td><?php echo con_6310_replace(esc_attr($allItber['description'])) ?></td>
                           </tr>
                     <?php
                        }
                     }
               ...
               <div class="con-6310-modal-form-footer">
                  ...
                  <input type="submit" name="add-edit-item-save" class="con-6310-btn-primary con-6310-pull-right con-6310-margin-right-10" value="Save" />
               </div>
            </form>
      ...
      ?>
      <div id="con-6310-rearrange-items-modal" class="con-6310-modal con-6310-display-none">
         <div class="con-6310-modal-content con-6310-modal-sm">
            <form action="" method="post">
               <?php wp_nonce_field("con-6310-nonce-rearrange-list") ?>
               ...
               <div class="con-6310-modal-body-form">
                  <div class="con-6310-filter-popup con-6310-padding-0-15">
                     ...
                     <ul id="con-6310-sortable-items">
                        <?php   
                           if($results) {
                              foreach ($results as $items) {
                                 echo "<li class='ui-state-default con-6310-ui-state-default' id='".esc_attr($items['id'])."'>".con_6310_replace(esc_attr($items['title']))."</li>";
                              }        
                           }
                           else {
                              echo "
               ...
               <div class="con-6310-modal-form-footer">
                  <button type="button" name="close" class="con-6310-btn-danger con-6310-pull-right">Close</button>
                  <input type="submit" name="rearrange-list-save" class="con-6310-btn-primary con-6310-pull-right con-6310-margin-right-10" id="con-6310-sortable-sub" value="Save" />
               </div>
            </form>
            <br class="con-6310-clear" />
         </div>
...
<?php
   ...
   function con_6310_description($ids, $description, $output = 1){
      if(!$description) return;
      if(!$output) return;
      echo "<div class='con-6310-template-".esc_attr($ids)."-description'>".esc_attr($description)."</div>";
   }

   function con_6310_number_format($number){
      $numbers = explode('.', $number);
      return ((isset($numbers[1]) && $numbers[1] > 0) ? strlen($numbers[1]) : 0);
   }
   
function con_6310_special_character_remove($data)
   {
      if($data == '') return ''; 
      else if(strlen($data) == 0) return ''; 
      while(strpos($data, "\\") !== false) {
   ...
   function con_6310_counter_prefix($data, $ids){
      if(!$data) return;
      echo "<div class='con-6310-counter-{$ids}-count-prefix'>".con_6310_special_character_remove($data)."</div>";
   }

   function con_6310_counter_suffix($data, $ids){
      if(!$data) return;
      echo "<div class='con-6310-counter-{$ids}-count-suffix'>".con_6310_special_character_remove($data)."</div>";
   }

   function con_6310_export_full_counter_number_plugin() {
      ...
      $item_table = 'con_6310_item';
      $style_table = 'con_6310_style';

      $path = wp_upload_dir();
	  $txtName = 'counter-number-' . time() . '.txt';
      ...
      foreach ( $data as $selectedData ) {
         ...
         $sqlData .= 
					...
					numbers_start='".esc_sql($selectedData['numbers_start'])."', 
					numbers='".esc_sql($selectedData['numbers'])."',
               ...
               commons='".esc_sql($selectedData['commons'])."',
               endnumbertype='".esc_sql($selectedData['endnumbertype'])."',
               selectoption='".esc_sql($selectedData['selectoption'])."',
               commentpost='".esc_sql($selectedData['commentpost'])."',
               leadingnumber = '".esc_sql($selectedData['leadingnumber'])."',
               sideimage = '".esc_sql($selectedData['sideimage'])."',
               styleid='".esc_sql($selectedData['styleid'])."'!@#$$#@!"; 
      }
      ...
      fwrite($fp, pack("CCC",0xef,0xbb,0xbf)); 
		fwrite($fp, $sqlData);				
		fclose($fp);
		echo '<a href="'.$path['url'].'/'.$txtName.'" target="_blank" id="export-counter-number-plugin">Download</a>';
   }
   
   

   function ano_6310_create_dropdown($list, $name, $sel = -1){
      echo "<select name='{$name}' class='con-6310-form-input' id='{$name}'>";
   ...
   function con_6310_common_form_pro_tag_add($templateId){
      if((int) $templateId == 1 || (int) $templateId == 2 || (int) $templateId == 3){  }else{
         echo '<span class="con-6310-pro">(Pro) <div class="con-6310-pro-text">This feature is available on the pro version only. You can view changes in the admin panel, not in the output.</div></span>';
       } 
   }

   function con_6310_end_number($value){
      if($value->endnumbertype == 2) {
         if($value->selectoption == 1){
            //post count
            return wp_count_posts( $post_type = 'post' )->publish + $value->leadingnumber;
         } else if($value->selectoption == 2){
            //page count
            return wp_count_posts( $post_type = 'page' )->publish + $value->leadingnumber;
         } else if($value->selectoption == 3){
            //page and post count
            return (wp_count_posts( $post_type = 'post' )->publish + wp_count_posts( $post_type = 'page' )->publish) + $value->leadingnumber;
         } else if($value->selectoption == 4){
            //comment count
            return get_comments(array('count'   => true))  + $value->leadingnumber;
         } else if($value->selectoption == 5){
            //comment count based on post
            $total = 0;
            $list = $value->commentpost ? explode(',', $value->commentpost) : [];
            if($list){
               foreach ($list as $val) {
                  $val = trim($val);
                  if(!$val) continue;
                  $total += get_comments(array('post_id' => $val, 'count' => true));
               }
            }
            return $total + $value->leadingnumber;
         } else if($value->selectoption == 6){
            return count_users()['avail_roles']['subscriber'] + $value->leadingnumber;
         }
      } 
      return $value->numbers;
   }

   function con_6310_save_data(){
      global $wpdb;
      $item_table = $wpdb->prefix . 'con_6310_item';
      $visitor = (int) sanitize_text_field($_GET['visitor']);
      $ids = (int) sanitize_text_field($_GET['id']);
      $wpdb->query($wpdb->prepare("UPDATE $item_table SET numbers = %d WHERE id = %d", $visitor, $ids));
      wp_die();
   }