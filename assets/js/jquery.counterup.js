...
window.addEventListener('load', function(){
  let allCounter = document.querySelectorAll(".con-6310-counter-number-attrs");
  
  if (allCounter.length) {
    for (let i = 0; i < allCounter.length; i++) {
      let separator = allCounter[i].getAttribute('data-con-6310-thousands-separator');
      let options = {
        useEasing: false,
        useGrouping: true,
        separator: separator,
      ...
      let start = allCounter[i].getAttribute("data-con-6310-start");
      let end = allCounter[i].getAttribute("data-con-6310-end");
      let decimal = allCounter[i].getAttribute("data-con-6310-decimal");
      let duration = allCounter[i].getAttribute("data-con-6310-duration");     
      let demo = new CountUp(ids, start, end, decimal, duration, options);
      demo.start();
      jQuery('#' + ids).addClass('con-6310-animation-completed');
    }
  }

  window.addEventListener('scroll', debounceEvents(function() {
    allCounter = document.querySelectorAll(".con-6310-counter-number-attrs");
    if (allCounter.length) {
      for (let i = 0; i < allCounter.length; i++) {
        let ids = '#' + allCounter[i].getAttribute("id");
        if(isScrolledIntoView(ids)) {
          if(!jQuery(ids).hasClass('con-6310-animation-completed')) {
            jQuery(ids).addClass('con-6310-animation-completed');
            let separator = allCounter[i].getAttribute('data-con-6310-thousands-separator');
            let options = {
              useEasing: false,
            ...
            let start = jQuery(ids).attr("data-con-6310-start");
            let end = jQuery(ids).attr("data-con-6310-end");
            let decimal = jQuery(ids).attr("data-con-6310-decimal");
            let duration = jQuery(ids).attr("data-con-6310-duration");
            let demo = new CountUp(allCounter[i].getAttribute("id"), start, end, decimal, duration, options);
            demo.start();
          }
        } else{
          jQuery(ids).removeClass('con-6310-animation-completed');
        }
      }
    }
  }, 50));
