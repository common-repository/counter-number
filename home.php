...
<table class="con-6310-table">
   ...
   foreach ($data as $value) {
       ...
       echo '<tr class="con-6310-row-select">';
       echo '<td>'.esc_attr($value['name']).'</td>';
       echo '<td>'.ucfirst(esc_attr($style_name[0])).' '.(int) esc_attr($style_name[1]).'</td>';
       echo '<td><span>Shortcode <input type="text" class="con-6310-6330-shortcode" onclick="this.setSelectionRange(0, this.value.length)" value="[con_6310_counter_number id=&quot;'.esc_attr($id).'&quot;]"></span>';
       echo '<td>
            <a href="'.admin_url("admin.php?page=con-6310-template-".esc_attr($temp)."&styleid=".esc_attr($id)).'" title="Edit"  class="con-6310-btn-success con-6310-margin-right-10 con-6310-first"><i class="fas fa-edit" aria-hidden="true"></i></a>
            <form method="post">
            ...
            <form method="post">
               '.wp_nonce_field('tss_nonce_field_delete').'
                     <input type="hidden" name="id" value="'.esc_attr($id).'">
                     <button class="con-6310-btn-danger con-6310-third"  title="Delete"  type="submit" value="delete" name="delete" onclick="return confirm(\'Do you want to delete?\');"><i class="far fa-times-circle" aria-hidden="true"></i></button>
            </form>
